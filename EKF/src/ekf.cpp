...
namespace Filters
{
	...
	void EKF::commence(const cv::Mat1d &x0, const cv::Mat1d &P0)
	{
		assert(1 == x0.cols);
	...
	const cv::Mat1d& EKF::predictState(const cv::Mat1d &F_t, const cv::Mat1d &Q_t)
	{
		...
		statePre = F_t_ * statePost0;
		errorCovPre = F_t * errorCovPost0 * F_t.t() + Q_t;
		errorCovPre = (errorCovPre + errorCovPre.t()) / 2;
		return statePre;
	}
	const cv::Mat1d& EKF::predictMeas(const cv::Mat1d &H_t, const cv::Mat1d &R_t)
	{
		measPre = H_t * statePre;
		S_t = H_t * errorCovPre * H_t.t() + R_t;
		S_t = (S_t + S_t.t()) / 2;
		K_t = errorCovPre * H_t.t() * S_t.inv();
		return measPre;
	}
	const cv::Mat1d& EKF::update(const cv::Mat1d &Y_t, cv::Point *maxLoc_ptr)
	{
	...
	const cv::Mat1d& EKF::update(const cv::Mat1d &Y_t, double P_D, double P_G, double lambda)
	{
		assert(0 <= P_D && P_D <= 1);
		assert(0 <= P_G && P_G <= 1);
	...
	cv::Mat1d EKF::innoMat(const cv::Mat1d &Y_t)
	{
		int n_y = Y_t.rows;
		int m_t = Y_t.cols;
	...
	cv::Mat1d EKF::assoProb(const cv::Mat1d &Nu_t, double P_D, double P_G, double lambda)
	{
		int n_y = Nu_t.rows;
		int m_t = Nu_t.cols - 1;
		if (0 == m_t) {
	...
	void EKF::equatePost0ToPost(void)
	{
