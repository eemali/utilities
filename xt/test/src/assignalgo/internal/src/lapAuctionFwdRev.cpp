...
namespace assignalgo
{
    ...
    std::tuple<xt::xarray<double>,
               xt::xarray<double>,
               xt::xarray<double>>
    lapAuctionFwdRev(const xt::xarray<double> &costMatrix,
                     xt::xarray<double> rowSoln,
                     xt::xarray<double> colSoln,
                     xt::xarray<double> colRedux,
                     double maxAuctions,
                        ...
                        xt::xarray<double> &colRedux)
    {
        ...
        while ((xt::sum(!isUnassigned)() < (nAssigned + 1)) && (priceWarIter < 1000u))
        {
            ...
            for (size_t freeRow : rowList) // Find the next row to assign
            {
                ...
                std::tie(bid, bestCol) = computeBid(costs, isNaNcolSoln);
                bid = bid + epsilon;
                ...
                colSoln.at(bestCol) = (double)freeRow;
                rowSoln.at(freeRow) = (double)bestCol;
