...
namespace assignalgo
{
                                                 ...
                                                 const xt::xarray<size_t> &costSize)
    {
        ...
        for (size_t i = 0; i < nSolutions; ++i)
        {
            ...
            {
                // CAVEAT: sortrows
				assignment1 = xt::sortrows(assignment1);
                // assignment1 = xt::view(assignment1, xt::keep(xt::argsort(xt::col(assignment1, 0))));
                std::tie(assignment1, std::ignore, std::ignore) = kbestRemoveUnassigned(assignment1, costSize);
            }
            for (size_t j = i + 1; j < nSolutions; ++j)
            {
                ...
                {
                    // CAVEAT: sortrows
					assignment2 = xt::sortrows(assignment2);
                    // assignment2 = xt::view(assignment2, xt::keep(xt::argsort(xt::col(assignment2, 0))));
					std::tie(assignment2, std::ignore, std::ignore) = kbestRemoveUnassigned(assignment2, costSize);
                }
                if (assignment1 == assignment2)
                {
                    if (std::get<3>(solutionList.at(i)) <= std::get<3>(solutionList.at(j)))
                    {
                        toClearIndex.at(j) = true;
                    }
                    else
                    {
